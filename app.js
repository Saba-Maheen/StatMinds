 ...
 * Handles role-based registration, state persistence, assessment execution, analytics rendering, and iGOT integration.
 */
...
const App = {
  ...
  init() {
    ...
    if (!this.currentProfile) {
      ...
      this.loadRoleQuestions(this.currentProfile.designation);
  ...
  /**
   * Load Questions based on Officer Role
   */
  loadRoleQuestions(role) {
    if (window.ROLE_QUESTION_BANKS && window.ROLE_QUESTION_BANKS[role]) {
      this.activeQuestions = [...window.ROLE_QUESTION_BANKS[role]];
    } else if (window.ROLE_QUESTION_BANKS && window.ROLE_QUESTION_BANKS["Senior Statistical Officer"]) {
      this.activeQuestions = [...window.ROLE_QUESTION_BANKS["Senior Statistical Officer"]];
    } else if (window.QUESTION_BANK) {
      this.activeQuestions = [...window.QUESTION_BANK];
    }
  },
  ...
  handleRoleChange() {
    const roleSelect = document.getElementById('reg-officer-role');
    const selectedRole = roleSelect ? roleSelect.value : 'Senior Statistical Officer';
    
    // Auto adjust department hint if default
    const deptInput = document.getElementById('reg-officer-dept');
    if (deptInput) {
      if (selectedRole.includes('GIS')) {
        deptInput.value = 'Geospatial Statistics & Remote Sensing Unit';
      } else if (selectedRole.includes('Data Analyst')) {
        deptInput.value = 'Data Quality & Innovation Division';
      } else if (selectedRole.includes('Field Survey')) {
        deptInput.value = 'Field Operations Division (FOD)';
      } else {
        deptInput.value = 'National Statistical Systems Training Academy (NSSTA)';
      }
    }
  },
  ...
  /**
   * Handle Registration & Login Action
   */
  handleRegisterAndLogin() {
    const name = document.getElementById('reg-officer-name')?.value || 'Ramesh Kumar';
    const id = document.getElementById('reg-officer-id')?.value || 'EMP-MOSPI-1082';
    const role = document.getElementById('reg-officer-role')?.value || 'Senior Statistical Officer';
    const dept = document.getElementById('reg-officer-dept')?.value || 'National Statistical Systems Training Academy (NSSTA)';
    const zone = document.getElementById('reg-officer-zone')?.value || 'HQ - New Delhi';

    // Create or update active profile
    this.currentProfile = {
      id: id,
      name: name,
      designation: role,
      ...
      email: `${name.toLowerCase().replace(/\s+/g, '.')}@mospi.gov.in`,
      experienceYears: 5,
      batch: "ISS Cadre Active",
      zone: zone,
      currentCompetencies: {}
    };

    // Load questions tailored to this role
    this.loadRoleQuestions(role);
    this.userAnswers = {};
    this.activeQuizIndex = 0;
    this.latestReport = null;

    this.saveStateToStorage();
    this.showToast(`Registered as ${name} (${role}). Exam calibrated.`, 'success');
  ...
  resetToLogin() {
    this.navigateTo('login');
    this.showToast('Officer session ended. Returned to registration portal.', 'info');
  },

  /**
   * 1. Render Profile View (Clean View - Baseline Ratings Removed)
   */
  renderProfileView() {
    if (!this.currentProfile) return;
    ...
    document.getElementById('profile-role-pill').textContent = this.currentProfile.designation;
    
    const initials = this.currentProfile.name.split(' ').map(n => n[0]).join('').substring(0, 2);
    document.getElementById('profile-avatar').textContent = initials;

    // Render core domains registered for this role
    const container = document.getElementById('profile-role-domains-list');
    if (!container) return;
    container.innerHTML = '';

    const requirements = window.STATS_DATA.roleRequirements[this.currentProfile.designation] || 
                         window.STATS_DATA.roleRequirements["Senior Statistical Officer"] || {};

    const domainDescriptions = {
      "Statistics": "National survey sampling, inferential tests, confidence bounds, and error minimization.",
      "Python Basics": "Iterables, list comprehensions, data structures, and statistical scripting automation.",
      "Data Handling": "Pandas DataFrame manipulation, missing value imputation, and survey deduplication.",
      "Data Visualization": "Matplotlib/Seaborn statistical charting, distribution plots, and dashboard publishing.",
      "AI/ML": "Supervised predictive modeling, feature engineering, and survey anomaly classification.",
      "GIS": "Coordinate systems, GeoPandas spatial joins, and thematic district choropleth mapping.",
      "Statistical Computing": "Vectorized computations with NumPy/SciPy, batch calculations, and pipeline validation."
    };

    Object.keys(requirements).forEach(domain => {
      const div = document.createElement('div');
      div.className = 'p-3.5 rounded-xl bg-slate-900/80 border border-slate-800 space-y-1';
      div.innerHTML = `
        <div class="flex items-center justify-between">
          <span class="font-bold text-slate-100 text-xs">${domain}</span>
          <span class="text-[10px] font-mono font-bold text-blue-400 bg-blue-950/60 px-2 py-0.5 rounded border border-blue-800/40">Benchmark: ${requirements[domain]}%</span>
        </div>
        <p class="text-[11px] text-slate-400 leading-relaxed">${domainDescriptions[domain] || 'Core operational competency required for official duties.'}</p>
      `;
  ...
  /**
   * 2. Render Skill Gap View (Clean Full-Width Benchmark - Assessment Objective Removed)
   */
  renderSkillGapView() {
    if (!this.currentProfile) return;

    const role = this.currentProfile.designation;
    const roleLabel = document.getElementById('skill-gap-role-label');
    ...
    const requirements = window.STATS_DATA.roleRequirements[role] || 
                         window.STATS_DATA.roleRequirements["Senior Statistical Officer"] || {};

    const scopeDetails = {
      "Statistics": "Sampling Theory, Stratified Design, Hypothesis Testing & Dispersion",
      "Python Basics": "List Comprehensions, Dictionaries, Lambda Functions & Logic",
      "Data Handling": "Pandas Operations, Missing-Value Handling & Data Cleaning",
      "Data Visualization": "Distribution Plots, Boxplots, Matplotlib & Heatmaps",
      "AI/ML": "Model Evaluation, Supervised Algorithms & Preprocessing",
      "GIS": "Spatial Joins, GeoJSON, CRS & Thematic Choropleths",
      "Statistical Computing": "Vectorized Computations, Batch Processing & Validation"
    };

    Object.keys(requirements).forEach(skill => {
      const required = requirements[skill];
      const scope = scopeDetails[skill] || "Standard operational competency assessment";

      const tr = document.createElement('tr');
      tr.className = 'hover:bg-slate-800/40 transition';
      tr.innerHTML = `
        <td class="py-3 px-4 font-bold text-white">${skill}</td>
        <td class="py-3 px-3 font-mono text-blue-400 font-bold text-sm">${required}% Required</td>
        <td class="py-3 px-3 text-slate-300 text-xs">${scope}</td>
        <td class="py-3 px-3"><span class="badge-good px-2.5 py-1 rounded text-[10px] font-bold">MANDATORY CORE</span></td>
      `;
  ...
  /**
   * 4. Render Quiz View (Calibrated to Officer Role)
   */
  renderQuizView() {
    if (this.currentProfile) {
      this.loadRoleQuestions(this.currentProfile.designation);
    ...
    if (!this.activeQuestions || this.activeQuestions.length === 0) {
      this.loadRoleQuestions("Senior Statistical Officer");
    }

    const roleTitleEl = document.getElementById('quiz-role-title');
    const mainHeadingEl = document.getElementById('quiz-main-heading');
    if (roleTitleEl && this.currentProfile) {
      roleTitleEl.textContent = `• ${this.currentProfile.designation} Examination`;
    }
    if (mainHeadingEl && this.currentProfile) {
      mainHeadingEl.textContent = `${this.currentProfile.designation} Competency Assessment`;
    }
  ...
  autoFillQuizAnswers(showNotification = true) {
    ...
    this.activeQuestions.forEach(q => {
      if (q.topic === "Data Handling") {
        if (q.subtopic && (q.subtopic.includes("Missing-value") || q.subtopic.includes("cleaning") || q.subtopic.includes("Manipulation"))) {
  ...
  submitAssessment() {
    ...
    const historyItem = {
      ...
      title: `Assessment ${this.assessmentHistory.length + 1} - ${this.currentProfile ? this.currentProfile.designation : 'Statistical'} Exam`,
  ...
  renderCompetencyChart() {
    ...
    const competencies = this.currentProfile && Object.keys(this.currentProfile.currentCompetencies).length > 0 
      ? this.currentProfile.currentCompetencies 
      : {
          "Statistics": 67,
          "Python Basics": 100,
          "Data Handling": 33,
